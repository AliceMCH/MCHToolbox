...
class ScaI2cCore : public Sca, public common::BitManipulation
{
  ...
  public:
    ...
    const sca_rgf_t I2C_S_7B_R  {0x86, 1, 2, BO_NORMAL};
...
class ScaI2c : public ScaI2cCore
{
  public:
    ...
    virtual uint64_t readMByte(uint32_t daddr, uint32_t raddr, uint32_t size) const {return 0;}
...
class ScaI2cGbtx : public ScaI2c
{
  ...
  public:
    ...
    uint64_t readMByte(uint32_t daddr, uint32_t raddr, uint32_t size) const override;
...
class ScaI2cSampa : public ScaI2c
{
  ...
  public:
     ...
     */
    i2c_freq_t setFreq() const override
  { setNbyte(1); return ScaI2cCore::setFreq(FREQ_400k); }
};
