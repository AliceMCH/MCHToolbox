...
bool
Sampa::writeRegister(uint8_t addr, uint8_t value, bool do_readback) const
{
  bool ok = false;
  for(int i = 0; i < nretries_+1; i++) {
    try {
      //std::cout<<"Sampa::writeRegister("<<(int)addr<<","<<(int)value<<"): iteration "<<i<<std::endl;
      uint8_t sd = i2c_.writeByte(static_cast<uint32_t>(addr), value);
    } catch( const gbt::ScaException& e ) {
      if( ((int)addr) == 0xe ) {ok = true; break;}
      std::cerr<<"Sampa::writeRegister("<<(int)addr<<","<<(int)value<<") iteration "<<i<<" failed, reason: "<<e.what()<<std::endl;
      continue;
    }
    if( do_readback && readback_ && addr != 0xe ) {
      uint8_t ret;
      if( !readRegister(addr, ret) ) continue;
      //std::cout<<"Sampa::writeRegister: addr="<<(int)addr<<"  value="<<(int)value<<"  ret="<<(int)ret<<"  iter="<<i<<std::endl;
      if( ret == value ) { ok = true; break; }
    } else {
      ok = true;
      break;
    }
...
{
  //std::cerr<<"Sampa::writeChannelRegister("<<(int)channel<<","<<(int)raddr<<","<<(int)value<<")"<<std::endl;
  bool ok = false;
  for(int i = 0; i < nretries_+1; i++) {
    //std::cout<<"Sampa::writeChannelRegister: iteration "<<i<<std::endl;
    if( !writeRegister(REG_CHRGADD, raddr, false) ) continue;
    if( !writeRegister(REG_CHRGWDATH, (value >> 8) & 0x3, false) ) continue;
    if( !writeRegister(REG_CHRGWDATL, value & 0xff, false) ) continue;
    //if( !writeRegister(REG_CHRGCTL, ((0 << 7) | (1 << 6) | ((channel == CH_BROADCAST ? 1 : 0) << 5) | (channel & 0x20)), false) ) continue;
    if( !writeRegister(REG_CHRGCTL, ((0 << 7) | (1 << 6) | (channel & 0x1f)), false) ) continue;
    if( readback_ ) {
      uint16_t ret;
      if( !readChannelRegister(channel, raddr, ret) ) continue;
      //std::cout<<"Sampa::writeChannelRegister: value="<<value<<"  ret="<<ret<<std::endl;
      if( ret == value ) { ok = true; break; }
    } else {
      ok = true;
      break;
...
Sampa::writeChannelRegisterMultiple(uint32_t channel_mask, uint8_t raddr, uint16_t value) const
{
  if (channel_mask == CHANNEL_MASK_ALL)
    return writeChannelRegisterBroadcast(raddr, value);
  else {
    uint8_t ch(0);
...
{
  writeRegister(REG_CHRGADD, raddr, false);
  writeRegister(REG_CHRGCTL, ((0 << 7) | (0 << 6) | (channel & 0x1f)), false);
  return (static_cast<uint16_t>(readRegister(REG_CHRGRDATH)) << 8) | readRegister(REG_CHRGRDATL);
}

bool
Sampa::readChannelRegister(uint8_t channel, uint8_t raddr, uint16_t& ret) const
{
  //std::cerr<<"Sampa::readChannelRegister("<<(int)channel<<","<<(int)raddr<<")"<<std::endl;
  if( !writeRegister(REG_CHRGADD, raddr, false) ) return false;
  if( !writeRegister(REG_CHRGCTL, ((0 << 7) | (0 << 6) | (channel & 0x1f)), false) ) return false;
  uint8_t ret1, ret2;
  if( !readRegister(REG_CHRGRDATH, ret1) ) return false;
  if( !readRegister(REG_CHRGRDATL, ret2) ) return false;
  ret = (ret1 << 8) | ret2;
  return true;
  //return (static_cast<uint16_t>(readRegister(REG_CHRGRDATH)) << 8) | readRegister(REG_CHRGRDATL);
}
