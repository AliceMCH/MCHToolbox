...
class Sampa
{
  ...
  public:
  Sampa(gbt::ScaI2c& i2c) : i2c_(i2c), readback_(false), nretries_(0) {}
  ...
  void set_readback(bool rb) { readback_ = rb; }
  void set_nretries(int n) { nretries_ = n; }
     ...
     *  @return Read value
     */
    inline bool readRegister(uint8_t addr, uint8_t& ret) const
    {
      bool ok = true;
      try {
    ...
    inline uint8_t readRegister(uint8_t addr) const
    {
      return i2c_.readByte(static_cast<uint32_t>(addr));
    }
    ...
    bool writeRegister(uint8_t addr, uint8_t value, bool do_readback = true) const;
     ...
     */
    uint16_t readChannelRegister(uint8_t channel, uint8_t raddr) const;
    bool readChannelRegister(uint8_t channel, uint8_t raddr, uint16_t& ret) const;
     ...
     */
    bool writeChannelRegister(uint8_t channel, uint8_t raddr, uint16_t value) const;
     ...
     */
    bool writeChannelRegisterMultiple(uint32_t channel_mask, uint8_t raddr, uint16_t value) const;

    /** Write to SAMPA channel specific register of all channels
     *  @param raddr Channel register address
     *  @param val Value to write to registers
     */
    inline bool writeChannelRegisterBroadcast(uint8_t raddr, uint16_t value) const { return writeChannelRegister(CH_BROADCAST, raddr, value); }
