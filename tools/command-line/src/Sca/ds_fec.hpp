...
class DsFecAdc
{
  ...
  public:
    ...
    static float convScaTemp(uint16_t adc_raw, float corr = 1.0) {
        //x=ADC_CH[31]/4 
        //TEMP_INTERN =(4300.0-6.0*x)/11.0
        float x = (float(adc_raw))/4;
        return ((4300.0-6.0*x)/11.0);
    ...
    static float convVtrx(uint16_t adc_raw, float corr = 1.0) {
        ...
        const float r1(2.0e3);
        const float r2(1.0e3);
        float v_rssi = corr*float(adc_raw)/4096;
        return 1e3/r1 * (-1 * (r1+r2)/r2 * v_rssi + v_cc);
      }

    /** Convert PT100 reading to temperature
     *
     *  @param adc_raw Raw ADC value
     *  @param corr Correction factor
     *  @return PT100 temperature (in ºC)
     */
    static float convPT100(uint16_t adc_raw, float corr = 1.0) {
      return (0.1782 * static_cast<float>(adc_raw) - 545.1);
      }
...
/** Overload output operator for DsFecAdc::result_t
 *  @param os The output stream to print to
 *  @param r Reference to DsFecAdc::result_t
 *  @return Output stream os
 */
std::ostream& operator<<(std::ostream& os, const DsFecAdc::result_t& r);
 ...
 *    GPIO, I2C, ...
 */
class DsFec : public common::BitManipulation
{
  public:
    static const uint8_t n_gbtx_{2};
    static const uint8_t n_sampa_{80};
  ...
  protected:
    ...
    std::array<std::unique_ptr<gbt::ScaI2c>, n_sampa_+1> i2c_sampa_;
  ...
  public:
    DsFec(DsFec const &) = delete;
    DsFec &operator=(DsFec const &) = delete;

    DsFec(common::HdlcCore& hdlc, uint8_t verbosity = 0) :
      ...
      sca_adc_(static_cast<gbt::ScaAdc*>(new gbt::ScaAdcVersion2(hdlc))),
      ...
      verbosity_(verbosity)
    {
      for (uint8_t i = 0; i < n_gbtx_; i++)
        i2c_gbtx_[i].reset(new gbt::ScaI2cGbtx(hdlc, i+8, cfg_i2c_addr_gbtx_[i]));
      for (uint8_t i = 0; i < n_sampa_; i++) {
        i2c_sampa_[i].reset(new gbt::ScaI2cSampa(hdlc, (i/10) + 0, cfg_i2c_addr_sampa_[i]));
        //std::cout<<"i2c_sampa_["<<(int)i<<"]="<<i2c_sampa_[i].get()<<std::endl;
        sampa_[i].reset(new Sampa(static_cast<gbt::ScaI2c&>(*i2c_sampa_[i])));
      }
      i2c_sampa_[n_sampa_].reset(new gbt::ScaI2cSampa(hdlc, 8, 0x50));

      for (uint8_t i = 0; i < n_sampa_; i++) {
        //std::cout<<"sampa_["<<(int)i<<"]="<<sampa_[i].get()<<std::endl;
      }
    }
     ...
     */
    uint32_t readUniqueID(std::ostream* const os = nullptr) const;

    /** Reads the unique ID of the SOLAR board
     *  @param os Pointer to output stream or nullptr if printing is not desired
     *  @return The SOLAR unique ID
     */
    uint32_t readTemperature(std::ostream* const os = nullptr) const;
     ...
     *  @return The SOLAR unique ID
     */
    bool sampaCheckConfiguration(std::string filename, int16_t sampa_id, uint8_t nRetry = 10, std::ostream* const os = nullptr) const;
     ...
     *  @see DsFecAdc::port_def_t
     *  @see DsFecAdc::result_t
     */
    void adcMeasureAll(std::ostream& os = std::cout)
      {
        const gbt::ScaAdc& adc = *(sca_adc_.get());
        for (auto& port : DsFecAdc::ports_)
          os << DsFecAdc::getInput(port.first, adc) << "\n";
        os << DsFecAdc::calcCurrent2r25(adc) << "\n"
           << DsFecAdc::calcCurrent3r25(adc) << "\n";
      }
    ...
    void adcMeasureInput(uint8_t input)
      {
        try {
          std::cout << DsFecAdc::getInput(input, *(sca_adc_.get())) << "\n";
        }
     ...
     */
    friend std::ostream& operator<<(std::ostream& os, const DsFec& f)
      {
        f.printIdString(os);
