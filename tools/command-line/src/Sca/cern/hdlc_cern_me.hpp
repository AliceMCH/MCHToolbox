...
class HdlcCernMe : public common::HdlcCore
{
  public:
    ...
    HdlcCernMe(const common::Bar& bar, uint8_t sca_idx = 0x0, bool request_trid_auto = false) :
      ...
      //std::cout<<"sca_idx: "<<(int)sca_idx<<std::endl;
        ...
        request_trid_ = 123; //getLastValidRequestTrid();

        // Set SCA core mux
        //printf("HdlcCernMe: setScaCoreMux(%d)\n", (int)sca_idx);
     ...
     */
    void sendSvlConnect() final override { startFrameTx(B_SEND_CONNECT); }

    /** Send supervisory level command "TEST" to SCA (see SCA manual p.11)
     */
    void sendSvlTest() final override { std::cout << __func__ << " not implemented for HdlcCernMe core\n"; }

    /** Send supervisory level command "RESET" to SCA (see SCA manual p.11)
     */
    void sendSvlReset() final override { startFrameTx(B_SEND_RST); }
  ...
  private:
    ...
    static const uint32_t REG_MON         {0x1c/4}; // {0x3c/4};    // Monitoring
      ...
      static const uint32_t REG_SCA_CORE_MUX {0x78/4};
    ...
    inline bool rxReady() const
      {
        //printf("rxReady(): %d\n", (int)bar_.readBit(base_addr_ + REG_CTRL2, B_CORE_BUSY));
        //printf("bar_.readRegister(%X) = %X\n", (int)(base_addr_ + REG_CTRL2), (int)bar_.read(base_addr_ + REG_CTRL2));
