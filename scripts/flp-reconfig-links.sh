...
for CRU in $(seq 0 31); do
    ...
    echo "xterm -bg black -fg white -geometry 100x45 -T \"cru-fec-config.sh $CRU $LINKS\" -hold -e ./cru-fec-config.sh $CRU $LINKS"
    xterm -bg black -fg white -geometry 100x45 -T "cru-fec-config.sh $CRU $LINKS" -hold -e ./cru-fec-config.sh $CRU $LINKS &
done
