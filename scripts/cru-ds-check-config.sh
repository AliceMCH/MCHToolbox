...
ERROR=0
for I in $(seq 1 1); do
    echo "../tools/command-line/build/ds-config -c \"${CRU_PCI_ADDR1}\" \"${CRU_PCI_ADDR2}\" $CFGFILE $RETRIES"
    time ../tools/command-line/build/ds-config -c "${CRU_PCI_ADDR1}" "${CRU_PCI_ADDR2}" $CFGFILE $RETRIES #>& sampa_dump_${CRU}.log
    RET=$?
    if [ x"$RET" = "x0" ]; then
	echo ""; echo "Check of $NENABLED sampa boards finished"
    else
	#exit 0
	...
	echo ""; echo "Check of $NENABLED sampa boards finished with errors"
	ERROR=1
    fi
done
